...
func _build_runtime_children(light_count: int) -> void:
    ...
    _light_phase.clear()
    _add_imported_lumen_overlays()
    _add_vein_overlays()
    _add_imported_wall_rims()
    _add_signal_lights(light_count)

func _add_imported_lumen_overlays() -> void:
    # Final floor art is mapped onto the spline ribbon itself, never onto a rectangular
    # tile. It therefore adds painterly microscopic variation while preserving every
    # widening, narrowing, curve, and branch generated by the arena graph.
    var texture := AssetResolverScript.floor_overlay(biome.id, int(corridor_data["seed"]))
    if texture == null:
        return
    ...
    for track in _tracks:
        ...
        overlay.name = "ImportedLumenOverlay"
        var lumen_polygon: PackedVector2Array = _ribbon_polygon(points, widths, -3.0)
        overlay.polygon = lumen_polygon
        overlay.uv = _ribbon_texture_uv(lumen_polygon, texture_size)
...
func _ribbon_texture_uv(polygon: PackedVector2Array, texture_size: Vector2) -> PackedVector2Array:
    # Polygon2D UVs are texture-space pixels. Use the same texel density in X and Y
    # instead of fitting one image to the ribbon bounds: a 96 px branch and a long
    # 900 px lumen therefore preserve the artist's painted aspect ratio. The masters
    # are validated seamless and the node enables repeat above, avoiding hard cards.
    var result := PackedVector2Array()
    var texture_span := maxf(maxf(texture_size.x, texture_size.y), 1.0)
    var texels_per_world_pixel := texture_span / 960.0
    var anchor := texture_size * 0.5
    for point in polygon:
        result.append(point * texels_per_world_pixel + anchor)
    return result
